...
num_bunny_particles_volume = bunny.num_particles_volume
...
bunny_normals = ti.Vector.field(dim, float)
ti.root.dense(ti.i, total_num_bunny_particles).place(old_bunny_positions, bunny_positions, bunny_velocities, bunny_normals)
...
@ti.func
def get_particle_surface(p_idx):
    # obj id
    particle_id = get_particle_obj(p_idx) - 2
    # bunny id
    b_idx = p_idx - num_fluid_particles - particle_id*num_bunny_particles
    # bool
    bool_surface = True
    if (b_idx < num_bunny_particles_volume):
        bool_surface = False
...
def prologue():
    ...
    # for i in bunny_positions:
    #     pos, nom = bunny_positions[i], bunny_normals[i]
    #     pos += nom * 0.1
    #     bunny_positions[i] = pos
...
def init_particles():
    ...
    for p_i in range(total_num_bunny_particles):
        if get_particle_surface(p_i):
            pos_i = bunny_positions[p_i] 
            ph_i = get_particle_obj(p_i)
            ...
            for j in range(particle_num_neighbors[p_i]):
                ...
                if ph_i == ph_j:
                    ...
                    tmp_counter += 1
            n = tmp / tmp_counter
            bunny_normals[p_i] = n/n.norm()
        else:
            bunny_normals[p_i] = ti.Vector([0.,0.,0.])
